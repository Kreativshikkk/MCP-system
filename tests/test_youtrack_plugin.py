...
class YouTrackPluginTests(unittest.TestCase):
 ...
 def test_issue_command_comment_link_sprint_and_work_item(self):
  with tempfile.TemporaryDirectory() as root:
   r=PluginRegistry();r.register(YouTrackPlugin());s=MCPSystem(Path(root),r);x=load_template_spec(Path("configs/templates/youtrack-default.toml"));s.create_template(x);e=s.create_environment_from_template(x.template_id);d=MCPDispatcher(s,e.id,actor="lead");self.assertEqual(len(d.list_tools()),20)
   def call(n,a):
