...
class YouTrackContractTests(unittest.TestCase):
 def test_selection_is_pinned(self):
  p=json.loads(Path("contracts/youtrack/provenance.json").read_text());s=json.loads(Path("contracts/youtrack/selected-operations.json").read_text());self.assertEqual(s["sourceSha256"],p["sourceSha256"]);self.assertEqual(len(s["operations"]),20);self.assertEqual(len({x["mcpTool"] for x in s["operations"]}),20)
